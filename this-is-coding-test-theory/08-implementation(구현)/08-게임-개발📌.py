...
메뉴얼에 따라 캐릭터를 이동시킨 뒤에, 
캐릭터가 방문한 칸의 수를 출력하는 프로그램을 만드시오.

cf.
첫째 줄에 맵의 세로 크기 N과 가로 크기 M을 공백으로 구분하여 입력한다.(3 <= N, M <= 50)

둘째 줄에 게임 캐릭터가 있는 칸의 좌표 (A, B)와 바라보는 방향 d가 
각각 서로 공백으로 구분하여 주어진다. 방향 d의 값으로는 다음과 같이 4가지가 존재한다.
- 0 : 북쪽
- 1 : 동쪽
- 2 : 남쪽
- 3 : 서쪽
...
output #1 이동을 마친 후 캐릭터 방문한 칸의 수
3

Tip
전형적인 시뮬레이션 문제로 삼성 공채코테에 자주 출제된다.
별도 알고리즘이 필요하기 보다 문제에 요구하는 내용을 오류없이 성실히 구현하면 된다.
그러나 소스코드로 옮기는 과정이 쉽지 않으므로 반복적인 숙달이 필요하다.

일반적으로 방향을 설정해 이동하는 문제는 dx, dy라는 별도 리스트를 만들고, 방향을 정하는 것이 효과적이다.
