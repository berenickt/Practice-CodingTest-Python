...
Advanced
1. 가장 큰 수를 k번, 두번째 큰 수를 더하면서 반복되는 수열이 발생한다.
2. 그렇다면? 반복되는 수열의 길이는? (K+1)이 된다.
3. 즉, M을 (K+1)로 나눈 몫이 수열이 반복되는 횟수가 된다.
4. 다시 여기에 K를 곱해주면 가장 큰 수가 등장하는 횟수가 된다.
5. 이떄 M이 (K+1)로 나누어 떨어지지 않는 경우도 고려하면, 가장 큰 수가 더해지는 횟수는 다음과 같다.
int(M / (k+1) * K + M % (K+1))
"""
