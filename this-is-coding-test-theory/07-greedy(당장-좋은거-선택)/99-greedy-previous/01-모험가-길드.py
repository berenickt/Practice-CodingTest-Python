...
output #1 여행을 떠날 수 있는 그룹 수의 최댓값
2

Tip
최대한 많은 그룹 수를 만드는 방법은, 최소한의 인원으로만 그룹을 구성한다.
1. 공포도를 기준으로 오름차순 (1 2 2 2 3)
2. 현재 그룹에 포함된 모함가의 수가 
    현재 확인 중인 공포도보다 크거나 같으면, 그룹으로 설정
"""
...
group_cnt = 0  # 총 그룹의 수
adv_cnt = 0  # 현재 그룹에 포함된 모험가의 수

for adv in adventurers:  # 공포도를 낮은 것부터 하나씩 확인하며
    adv_cnt += 1  # 현재 그룹에 해당 모험가를 포함시키기

    # 현재 그룹에 포함된 모험가의 수가 현재의 공포도 이상이라면, 그룹 결성
    if adv_cnt >= adv:
        group_cnt += 1  # 총 그룹의 수 증가시키기
        adv_cnt = 0  # 현재 그룹에 포함된 모험가의 수 초기화
