...
for ch in postfix:
  if A <= ord(ch) <= Z: # 피연산자인 경우, 해당 피연산자 값을 스택에 추가
    ...
    if ch == '+': s.append(op1+op2)   # 덧셈 연산을 수행하고 결과를 스택에 추가
    elif ch == '-': s.append(op1-op2) # 뺄셈 연산을 수행하고 결과를 스택에 추가
    elif ch == '*': s.append(op1*op2) # 곱셈 연산을 수행하고 결과를 스택에 추가
    elif ch == '/': s.append(op1/op2) # 나눗셈 연산을 수행하고 결과를 스택에 추가
